...
```
ECC/
├── ECC_catboost_mondrian_cqr.py   ← Main implementation (start here)
├── images                          ← Plots and CSVs generated 
└── other_implementations/
    ├── backend_comparison.py     ← ET+GBR vs CatBoost vs NGBoost vs LightGBM vs TabPFN
    ├── mondrian_cqr_moe.py       ← Mondrian CQR + Mixture-of-Experts (4 variants)
    ├── ECC.py                     ← Baseline pipeline (ET+GBR + plain CQR)
    └── tabpfn_moe_router.py      ← TabPFN as MoE router experiment
```
...
The main (`ECC_catboost_mondrian_cqr.py`) implements a two-stage pipeline:
